...
import { AUDIO_FILES, BED_HIGH, BED_LOW, CALLOUT_SOUND, SOUND_BANK, specFor, voiceLifetimeS, type SoundId } from './bank';
...
describe('the sound bank covers the event stream', () => {
  ...
  it('never lets a clip talk over itself, which is what keeps one player per clip enough', () => {
    for (const spec of SOUND_BANK) {
      const dur = CLIP_MEASUREMENTS[spec.id].durationS;
      expect(spec.minGapS, `${spec.id}: minGapS ${spec.minGapS} must exceed its own ${dur} s length`).toBeGreaterThanOrEqual(dur + 0.05);
    }
  });
...
describe('the rendered WAVs', () => {
  it('exist, one per clip, and nothing else', () => {
    expect(AUDIO_FILES).toHaveLength(SOUND_BANK.length + 2);
  ...
  it('spans a designed ladder rather than a random spread', () => {
    const oneShots = SOUND_BANK.map((s) => CLIP_MEASUREMENTS[s.id].rmsDb);
...
describe('the belief gate', () => {
  it('stays silent while the engine does not stand behind the reading', () => {
    const c = clock();
    const r = recorder();
    const feel = new DriftFeel({ now: c.now });
    feel.attach(r.sound, r.haptic);
  ...
  it('still announces the facts that are true whether or not a slide is believed', () => {
    ...
    const feel = new DriftFeel({ now: c.now });
    feel.attach(r.sound, r.haptic);
    const unbelievable = { mount: 'loose', physics: 'ok', gps: 'none', message: '', believable: false } as LiveFrame['integrity'];
...
describe('a real run, through the real pipeline', () => {
  /** Push a simulated run through the engine and the feel layer at 1x. */
  function drive(opts: Parameters<typeof simulateRun>[1]) {
    const run = simulateRun('harbor', { seed: 1, laps: 1, ...opts });
  ...
  it('says nothing at all about a hand-held recording', () => {
    // The drive display refuses to bloom for a run the scorer is not paying for; the feel layer
    // refuses to speak. Silence IS the signal, and the HUD's NOT SCORING carries the reason.
    const { at, r, feel } = drive({ looseness: 1, gpsDropouts: true });
