  ...
  if (track && track.laps.length) {
    for (const lap of track.laps) {
      ...
      if (inLap.length >= o.cleanLapMinDrifts && !inLap.some((d) => d.spun)) {
        ...
        // AND it is worth only as much of itself as the monitor believed. `last.total > 0` is a
        // question about the lap's HISTORY; it was the only question here, and a harbor run at
        // looseness 0 paid 1,725 points across two crossings the monitor refused — on frames the
        // HUD was simultaneously stamping NOT SCORING. `lapBelief` is the SAME expression
        // `LiveScorer.onLapCompleted` uses, over the same two per-drift durations, so the live
